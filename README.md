...
INV-202303-00257	4
INV-202501-00283	4
Samm 3. Loe kokku duplikaatsete ridade arv: SELECT COUNT(*) AS duplikaat_read FROM sales_test WHERE id NOT IN ( SELECT MIN(id) FROM sales_test GROUP BY invoice_id );

duplikaat_read
5116
Samm 4. Leia NULL väärtused kriitilistes väljades: SELECT COUNT() FILTER (WHERE customer_id IS NULL) AS null_customer_id, COUNT() FILTER (WHERE sale_date IS NULL) AS null_sale_date, COUNT(*) FILTER (WHERE total_price IS NULL) AS null_total_price FROM sales_test;

null_customer_id	null_sale_date	null_total_price
1487	0	0
Samm 5. Kontrolli kuupäevade formaati — kas on tuleviku kuupäevi? SELECT COUNT(*) AS tuleviku_kuupaevad FROM sales_test WHERE sale_date > CURRENT_DATE;
...
SQL päringud / failid
Lisa lingid https://supabase.com/dashboard/project/jrkmbzgrdxffhdmbtvey/sql/7464cfd4-9fde-4588-a4ff-2298bd66548c
